...
class TrajectoryRecorder:
    """Collect and atomically write one JSON trajectory per request."""
    ...
    def __init__(
        ...
        *,
        ...
        filename = (
            ...
            f"-{_safe_id(task_id, 'task')}-{_safe_id(context_id, 'context')}.json"
        )
    ...
    def record_graph_update(self, event: dict[str, Any], formatted_event: str = "") -> None:
        ...
        for message in _iter_messages(event):
            self.data["tool_events"].extend(_tool_event_from_message(message))
